...
| 18 | **Observability & incident drills** — real `logs`, events, metrics, timed "3am pager" mode | M | ✅ 2026-07-18 — module m21 (`src/data/obsLabs.js` + `ObsLabs.jsx`): the engine gained three things the sim had been faking — per-container log buffers written by the reconcile loop itself (startup banners, access-log heartbeat, probe-failure warnings, OOM kills, crash last-words), rotated into `prevLogs` on restart so `logs --previous` reads the instance that actually died; `addEvent` aggregation into one row per (object, reason, message) with a real `count`/`firstTimestamp`/`lastTimestamp`, replacing a dedupe rule that only looked at the immediately preceding event; and a rolling per-pod metrics ring with CPU that moves plus a `setLoad()` knob. `kubectl` caught up: `logs` takes `--tail/--since/--timestamps/-f` (`-f` parses as `--follow` only under `logs`) and a truthful `--previous`, `get events` grew a COUNT column with `--sort-by`/`--field-selector`, `describe` renders "(x47 over 6m)", and `top` reads the ring with `--sort-by=cpu\|memory`. A new pure `src/sims/k8s/slo.js` folds the rings into availability + error-budget burn. Three labs (logs, events, metrics/SLO — the last a healthy playground whose panel injects a CPU spike that costs no budget and a broken replica that costs all of it) plus **incident mode**: `src/data/incidents.js` pages a random Troubleshooting scenario with the symptom only, runs a time-to-diagnose and a time-to-resolve clock, and grades with the scenario's own checks — the root-cause choices are other incidents' causes, so every distractor is a real failure mode. `obsLabs.test.js` + `incidents.test.js`. |
| 19 | **Pedagogy layer** — tracks, prerequisites, next-step, spaced repetition | M | ✅ 2026-07-18 — module metadata is now one registry (`src/data/modules.js`) instead of four hand-synced copies (`App.jsx` ids, `Sidebar.jsx` titles/order/pills, `router.js` slugs, `roadmap.js` MODNAMES); `src/data/tracks.js` adds five ordered goals (just Docker / CKAD / CKA / CKS / senior) and `src/data/pedagogy.js` folds progress into them — `moduleStats()` is the per-module completion signal lifted out of Sidebar's inline `pills` object, so it now feeds prerequisite checks and the next-step bar as well as the sidebar, and is unit-testable. Gating is **advisory**: an unmet prerequisite shows 🔒 plus a "do first" tooltip naming what to go back to, but nothing is ever blocked or hidden. Reading modules have no missions, so visiting one is their completion signal (`dk8sseen`); the goal persists as `dk8sgoal`. A goal picker sits on the Roadmap (the default route) and a "next →" bar renders once in `App.jsx` below the module sections. Spaced repetition: `src/data/leitner.js` — five boxes at 1/3/7/21 days, question identity by FNV-1a hash of the English text (the bank has no ids and its array index shifts under filtering), sessions capped at 20 with cards already in a box served before never-seen ones so a lapse can't be starved by new material; the quiz gained a Practice/Review mode switch and persists the deck as `dk8sdeck`, leaving `recordQuiz`/`quizStats` untouched so the readiness dashboard is unaffected. Doc links reached the last three surfaces: hand-authored `docs[]` on all 11 scenarios and 30 native exam tasks (scenario-derived tasks inherit), and a per-domain table (`src/data/docLinks.js`) covering all 96 quiz questions, rendered by a shared `DocLinks.jsx` extracted from `LabRunner`. `pedagogy.test.js` + `leitner.test.js` + `docLinks.test.js`. |

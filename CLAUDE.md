...
- **Reading-module convention (m1/m3/m5/m7/m8/m9):** content keys are **named per topic** (`helmCard`, `qosCard`, …) — no anonymous `cards[]` arrays — so module JSX can interleave widgets and links per topic. Widget **chrome** (titles, button labels, at-rest hints) lives as named keys in both content files rendered via `<Rich>`; widget **narration** is inline EN constants in the module JSX rendered via `<Html html={tr(lang, str)}/>` with `[EN, KO]` pairs in `KO_STR`. A card body renders inside a shared wrapper as `<Rich content={c.xCard[0].c}/>` when a widget/link joins it. `src/__tests__/contentParity.test.js` guards EN/KO key sets, value types, and positional-array lengths (`_ncards` is vestigial and unenforced).
- **`@src/components/PracticeLink.jsx`** — `<PracticeLink to="m19" sub="helm" blurb={{en,ko}}/>`: the reading-topic → drill deep link (renders the target's live progress pill via `moduleStats`, navigates with `useRoute`). Every reading topic that has a matching drill tab should end with one; blurbs are `{en,ko}` props, not KO_STR entries.
